"""Quick visual test for debug overlay positioning. Run with: uv run python tests/test_overlay.py
Controls:
  Arrow keys  — move position
  + / -       — increase / decrease font height
  q / Escape  — quit and print final values

Wrapped in an `if __name__ == "__main__"` guard so that pytest can collect this
file without opening a window and blocking on the event loop.
"""
